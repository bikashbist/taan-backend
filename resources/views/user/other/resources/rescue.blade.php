@extends('user.user_dashboard')
@section('content')
    <section class="about-us gallery-page ">
        <div class="section-bg-banner">
            <div class="hero-bg">
                <img src="{{ asset('user/images/layout-img/page-title.webp') }}" alt="bg">
            </div>
            <div class="container">
                <div class="section-hero-title">
                    <p class="text-white"> Welcome to!</p>
                    <h1 class="text-white">Rescue Procedure 2075</h1>


                </div>
                <img class="page-title-icon" src="{{ asset('user/images/layout-img/icon-page-title.png') }}" alt="icon">
            </div>
        </div>

    </section>

    <section class="newsletter py-lg-5 py-3">
        <div class="container">
            <div class="newsletter__details">
                <div class="row mb-4 g-0 ">
                    <div class="col-lg-5">
                        <iframe src="https://www.taan.org.np/public/files/download/download_Rescue_2075.pdf" width="100%" height="100%">
                            
                        </iframe>
                    </div>
                    <div class="col-lg-7">
                        <div class="newsletter__file h-100 p-5">
                          
                        <h3 > पर्यटकखोज,उद्धार, उपचार तथा अनुगमन सम्बन्धी कार्यविधि, २०७५</h3>

                        <p >Tej Bahadur Gurung, 1st Vice President of TAAN and coordinator of TAAN Lhosar Cultural
                            Program 2016, the association is organizing the festival in Hyatt Regency Kathmandu to
                            make it more managed and entertaining. </p>
                            <a class="read-more text-white" href="https://www.taan.org.np/public/files/download/download_Rescue_2075.pdf" download>Downloads</a>
                           

                        </div>
                    
                           
                       
                    </div>

                


                </div>
                <div class="row mb-4 g-0 ">
                    <div class="col-lg-5">
                        <iframe src="{{ asset('user/images/layout-img/icon-page-title.png') }}" width="100%" height="100%">
                            
                        </iframe>
                    </div>
                    <div class="col-lg-7">
                        <div class="newsletter__file h-100 p-5">
                          
                        <h3 > पर्यटकखोज,उद्धार, उपचार तथा अनुगमन सम्बन्धी कार्यविधि, २०७५</h3>

                        <p >Tej Bahadur Gurung, 1st Vice President of TAAN and coordinator of TAAN Lhosar Cultural
                            Program 2016, the association is organizing the festival in Hyatt Regency Kathmandu to
                            make it more managed and entertaining. </p>
                            <a class="read-more text-white" href="https://www.taan.org.np/public/files/download/download_Rescue_2075.pdf" download>Downloads</a>
                           

                        </div>
                    
                           
                       
                    </div>

                


                </div>
             
               
            </div>
    </section>
@endsection
